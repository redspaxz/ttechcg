...
$assert(CloudflareRequestTrust::contains('192.0.2.40'), 'Hosting providers should be able to explicitly allowlist an intermediate trusted reverse proxy CIDR.');
putenv($previousTrustedProxyCidrs === false ? 'CLOUDFLARE_TRUSTED_PROXY_CIDRS' : 'CLOUDFLARE_TRUSTED_PROXY_CIDRS=' . $previousTrustedProxyCidrs);
...
$assert(substr_count($fullReport->body(), 'class="report-chart"') >= 9 && str_contains($fullReport->body(), 'Cash settlement, last 3 months') && str_contains($fullReport->body(), 'Shipments per month') && str_contains($fullReport->body(), 'Cash recorded per month (XAF)'), 'The full report should render server-side charts for settlement, market pairs, monthly trend, destinations, senders and loyalty.');
$assert(str_contains($fullReport->body(), '<li><i style="background:#2a78d6" aria-hidden="true"></i>Latest period</li>') && str_contains($fullReport->body(), '<li><i style="background:#898781" aria-hidden="true"></i>Previous period</li>'), 'Two-series report charts should carry a legend so identity never depends on color alone.');
$assert(\App\Modules\Pickupsheet\UI\ReportCharts::niceTicks(81, true) === [0, 25, 50, 75, 100] && \App\Modules\Pickupsheet\UI\ReportCharts::niceTicks(1, true) === [0, 1] && \App\Modules\Pickupsheet\UI\ReportCharts::niceTicks(0) === [0, 1], 'Report chart axes should use clean integer ticks for counts and a safe scale for empty data.');
$assert(\App\Modules\Pickupsheet\UI\ReportCharts::compact(1174500) === '1.2M' && \App\Modules\Pickupsheet\UI\ReportCharts::compact(500000) === '500K' && \App\Modules\Pickupsheet\UI\ReportCharts::compact(25) === '25', 'Report chart ticks should compact large XAF values.');
$escapedChart = \App\Modules\Pickupsheet\UI\ReportCharts::horizontalBars([['label' => '<script>alert(1)</script>', 'value' => 3, 'display' => '3']], 'Sender chart');
$assert(!str_contains($escapedChart, '<script>') && str_contains($escapedChart, '&lt;script&gt;'), 'Report chart labels and tooltips should escape customer-controlled names.');
$assert(\App\Modules\Pickupsheet\UI\ReportCharts::horizontalBars([], 'Empty') === '' && str_contains(\App\Modules\Pickupsheet\UI\ReportCharts::shareBar([['label' => 'Paid', 'value' => 0, 'display' => '0', 'color' => '#2a78d6']], 'Empty settlement'), 'fill="#e1e0d9"'), 'Report charts should degrade to an empty track or nothing when there is no data.');
$zeroColumns = \App\Modules\Pickupsheet\UI\ReportCharts::columns([['label' => 'Jan 2026', 'value' => 0, 'display' => '0'], ['label' => 'Feb 2026', 'value' => 0, 'display' => '0']], 'Zero months', true);
$assert(str_contains($zeroColumns, '>Jan</text>') && !str_contains($zeroColumns, 'NAN') && !str_contains($zeroColumns, 'INF'), 'Monthly charts should render zero-activity months without invalid geometry.');
...
$assert(is_string($styles) && str_contains($styles, ".legal-grid aside strong { color: var(--ink); }") && str_contains($styles, ".pickup-admin-workspace .pickup-back,"), 'Dark pages should keep the privacy update date and admin header links readable.');
$assert(is_string($styles) && str_contains($styles, ".pickup-form .pickup-consent { display: grid; grid-template-columns: 20px minmax(0, 1fr);") && str_contains($styles, ".shipment-editor-note {"), 'The pickup form should render compact help notes and an inline consent checkbox.');
$assert(is_string($styles) && str_contains($styles, "    .pickup-login-shell { min-height: 0; }\n    .pickup-login-panel { justify-content: flex-start; }") && str_contains($styles, ".pickup-back { flex: 1 1 calc(50% - 7px);"), 'Stacked login panels and workspace header links should avoid large mobile gaps.');
...
$assert(is_string($bootstrap) && str_contains($bootstrap, "ini_set('display_errors', !\$isProduction") && str_contains($bootstrap, "zend.exception_ignore_args', '1'") && str_contains($bootstrap, "header_remove('X-Powered-By')"), 'Production should suppress browser error detail, omit exception arguments, and hide the PHP signature.');
$assert(is_string($bootstrap) && str_contains($bootstrap, "\$config['app_url'] = 'https://ttechcg.com'") && str_contains($bootstrap, "['scheme'] ?? null) !== 'https'"), 'Production should fail safely to the canonical HTTPS origin when APP_URL is malformed or insecure.');
$assert(is_string($bootstrap) && str_contains($bootstrap, '$localLoginSecurityReady') && str_contains($bootstrap, '$localMfa->isEnabled() && $localMfa->isConfigured()'), 'Production local login should fail closed unless local MFA is enabled and operational.');
$assert(is_string($bootstrap) && str_contains($bootstrap, 'new SecurityDataRetention('), 'Bootstrap should schedule bounded security evidence retention when MySQL is available.');
...
$assert(is_string($cpanelDeployment) && str_contains($cpanelDeployment, 'test -s ${DEPLOYPATH}.env') && str_contains($cpanelDeployment, 'chmod 600 ${DEPLOYPATH}.env'), 'cPanel deployment should require and protect the server-managed environment file.');
$assert(is_string($apacheConfig) && str_contains($apacheConfig, 'Options -Indexes -MultiViews') && str_contains($apacheConfig, 'LimitRequestBody 13631488') && str_contains($apacheConfig, 'X-Permitted-Cross-Domain-Policies'), 'Apache should deny negotiation and oversized requests while applying its defense-in-depth headers.');
$assert(is_string($workflow) && str_contains($workflow, 'php bin/security-check.php'), 'Continuous verification should reject tracked secrets and unpinned workflow dependencies.');
$assert(is_string($environmentLoader) && str_contains($environmentLoader, "'/^[A-Z][A-Z0-9_]{0,127}$/'") && str_contains($environmentLoader, 'strlen($value) > 8192'), 'Environment loading should accept only bounded conventional variable names and values.');
$assert(is_string($securityPolicy) && str_contains($securityPolicy, 'Reporting a vulnerability') && is_string($isoMapping) && str_contains($isoMapping, 'not an ISO/IEC 27001 certificate'), 'The repository should define private vulnerability reporting and accurately scope its ISO control mapping.');
