                ...
                <table class="records-user-table">
                    ...
                    <?php foreach ($accounts as $account): ?>
                        ...
                        <tr class="records-user-summary-row" data-active="<?= $account->active ? 'true' : 'false' ?>">
                            ...
                            <td class="records-user-action-cell">
                                <div class="records-user-row-actions">
                                    <button class="button button-dark" type="button" data-user-edit-toggle="<?= $e($editorId) ?>" aria-controls="<?= $e($editorId) ?>" aria-expanded="false">Edit</button>
                                    <form method="post" action="<?= $e($basePath) ?>/dhl/pickupsheet/submissions/users/status" data-user-status-form data-account-name="<?= $e($account->fullName()) ?>">
                                        <input type="hidden" name="_token" value="<?= $e($csrfToken) ?>">
                                        <input type="hidden" name="id" value="<?= $e($account->id) ?>">
                                        <input type="hidden" name="active" value="<?= $account->active ? '0' : '1' ?>" data-user-target-active>
                                        <input type="hidden" name="confirm_status" value="0" data-confirm-user-status>
                                        <button class="button <?= $account->active ? 'records-user-disable' : 'records-user-enable' ?>" type="submit"><?= $account->active ? 'Disable' : 'Enable' ?></button>
                                    </form>
                                    <?php if ($localMfaConfigured && $mfaEnrolled): ?><a class="button records-user-mfa-reset" href="<?= $e($basePath) ?>/dhl/pickupsheet/submissions/users/mfa/reset?id=<?= $e($account->id) ?>">Reset 2FA</a><?php endif; ?>
