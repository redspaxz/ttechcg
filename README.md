...
The pickup-sheet entry form remains public, and “Checked by” is a required field entered with each shipment. Stored records are fail-closed and protected by environment-backed RBAC over HTTPS Basic authentication. A `viewer` can list and paginate records, an `operator` can also print and export, and an `admin` has every records permission plus the reserved account-management permission. Password hashes exist only in the server-managed `.env`. Excel downloads contain only shipment-detail columns, shipment rows, and a bold shipment-total row; pickup-sheet header metadata and sequential row numbering are excluded. The records view uses database-backed pagination at 10 sheets per page, with progressively enhanced AJAX loading, a visible spinner, browser-history support, and standard links as a no-JavaScript fallback. Successful, forbidden, and denied records access is security-logged using hashed actor, client, and record identifiers. Legacy `/pickupsheet` URLs redirect permanently to the new `/dhl/pickupsheet` location.
...
Before deploying the hardened records access, run `php bin/generate-records-credentials.php USERNAME ROLE` on a trusted local computer once for each account. Valid roles are `viewer`, `operator`, and `admin`. Join the generated RBAC entries with semicolons and put the result in one quoted cPanel File Manager setting, for example `PICKUPSHEET_RBAC_USERS='viewer1|viewer|HASH;operator1|operator|HASH;admin1|admin|HASH'`, inside `/home/ttecwymc/public_html/.env`. Keep the generated passwords only in a password manager. The older `PICKUPSHEET_RECORDS_USER` and `PICKUPSHEET_RECORDS_PASSWORD_HASH` settings continue to work as one admin account during migration. Also ensure `APP_ENV=production` and `RUN_MIGRATIONS=false`. The records routes return `401` until at least one valid account is configured; the public pickup entry form continues to operate.
