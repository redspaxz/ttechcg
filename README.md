...
Set either value to `false` to hide that method and reject new requests to its authentication endpoint. Existing authenticated sessions retain their normal idle and absolute expiry; disable an individual local account when immediate account revocation is required. Local login defaults to enabled for backward compatibility; JumpCloud remains disabled until its full OIDC and RBAC configuration validates. Keep at least one direct method enabled unless Cloudflare Access is deliberately the required identity boundary. If neither direct method is available, the login route fails closed with HTTP `503`. The administrator user-management page reports the effective status but cannot modify `.env` secrets or security switches.
...
For production records access, configure every enabled authentication method in `/home/ttecwymc/public_html/.env`: a securely hashed local administrator when local login is enabled, and the OIDC client plus three role groups when JumpCloud is enabled. Also ensure `APP_ENV=production` and normally keep `RUN_MIGRATIONS=false`.

Edit `/home/ttecwymc/public_html/.env` in cPanel File Manager and set `PICKUPSHEET_LOCAL_LOGIN_ENABLED` and `JUMPCLOUD_OIDC_ENABLED` directly; no cPanel CLI is required. Never commit the client secret. PHP must have `curl` and `openssl` enabled for JumpCloud. After saving `.env`, open `/dhl/pickupsheet/login` in a private browser window and test every enabled sign-in method before closing an existing administrator session.
