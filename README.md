...
Every Pickupsheet screen is protected by the dedicated session login portal at `/dhl/pickupsheet/login`; HTTP Basic credentials no longer grant application access. JumpCloud OIDC can provide SSO and derive `admin`, `operator`, or `viewer` from approved JumpCloud groups. The authorization-code flow uses state, nonce, PKCE S256, an exact callback URI, regional issuer allowlisting, signed RS256 ID-token verification against JumpCloud JWKS, UserInfo subject matching, and required verified email/name claims. Accounts without a mapped group fail closed. Local environment and database-backed accounts remain available as a controlled recovery path until `JUMPCLOUD_OIDC_LOCAL_LOGIN=false` is set. Login IDs may be either usernames or email addresses. Every managed account requires a first name, last name, role, and explicit Active/Inactive status. The authenticated account's full name is displayed as the read-only Check By value and is enforced again by the server on create and edit, so a submitted value cannot impersonate another checker. Every new sheet starts `open`. A `viewer` can create, list, and paginate records but cannot edit them. An `operator` can create, view, edit, print, export, and change an open sheet to `paid`. An `admin` receives the activity dashboard, KPI graph, complete record visibility, edit/print/export access, the open-to-paid action, audited soft deletion, and user management. Edits preserve immutable references and consent metadata, recalculate totals, and write pseudonymous before-and-after snapshots. Paid transitions and deletions are separately audited; deleted records disappear from active views without destroying their database history. Local administrators are bootstrap accounts defined in the server-managed `.env`; from the RBAC area a local administrator can confirm the current password and store a secure replacement hash in MySQL, which immediately invalidates the previous password and session. JumpCloud passwords and MFA remain managed in JumpCloud. The same area creates database-backed recovery viewer/operator accounts and resets their login ID, names, role, password, or account status. Passwords are stored only as one-way hashes, sessions expire after one hour idle or eight hours total, and name/role/password/status changes invalidate managed-user sessions. Excel downloads contain only shipment-detail columns, shipment rows, and a bold shipment-total row; pickup-sheet header metadata and sequential row numbering are excluded. The records view uses database-backed pagination at 10 sheets per page, with progressively enhanced AJAX loading, a visible spinner, browser-history support, and standard links as a no-JavaScript fallback. Successful, forbidden, and denied records access is security-logged using hashed actor, client, and record identifiers. Legacy `/pickupsheet` URLs redirect permanently to the new `/dhl/pickupsheet` location.
...
Copy the client ID and the one-time client secret into the server-managed `.env`, then set the matching `JUMPCLOUD_OIDC_*` and `JUMPCLOUD_RBAC_*` values shown in `.env.example`. Use the issuer for the JumpCloud organisation's region: US `https://oauth.id.jumpcloud.com/`, EU `https://oauth.id.eu.jumpcloud.com/`, or India `https://oauth.id.in.jumpcloud.com/`. Keep `JUMPCLOUD_OIDC_LOCAL_LOGIN=true` through the first successful admin SSO test. Set it to `false` only after confirming a member of the admin group reaches the dashboard. JumpCloud users must also be authorised to the SSO application; application access is denied by default.

OIDC maps identities at sign-in and does not copy JumpCloud users into MySQL. Group or account changes therefore take effect on the next login or when the existing Pickupsheet session expires (one hour idle, eight hours absolute). Local recovery accounts remain separately managed unless local login is disabled.
...
For JumpCloud, edit `/home/ttecwymc/public_html/.env` in cPanel File Manager and paste the OIDC values directly; no cPanel CLI is required. Never commit the client secret. PHP must have `curl` and `openssl` enabled. After saving `.env`, start at `/dhl/pickupsheet/login` and test the JumpCloud button in a private browser window.
