...
This release uses migrations `005_create_pickup_records_users.sql` through `012_create_pickup_customers.sql`. Authenticated workflows initialize missing account, audit, lifecycle, administrator-credential, user-name, session-activity, security-event, and CRM customer storage when the runtime MySQL user has the required schema permissions, supporting cPanel accounts without CLI access. Login and unauthenticated requests cannot create schemas. If the runtime MySQL user does not have `CREATE` and `ALTER` permission, use cPanel phpMyAdmin to execute migrations `005` through `012`, then keep `RUN_MIGRATIONS=false`.
