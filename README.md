...
Before deploying this hardening to production, verify that JumpCloud or Cloudflare Access is operational, or set `PICKUPSHEET_LOCAL_MFA_ENABLED=true` with a valid `PICKUPSHEET_MFA_ENCRYPTION_KEY` and MySQL migration 015 available. Otherwise local sign-in intentionally remains unavailable. The root `.htaccess` marks requests whose underlying TCP peer is a published Cloudflare edge, which preserves trusted forwarding headers when cPanel/LiteSpeed restores `REMOTE_ADDR` to the visitor address. If the hosting provider inserts another reverse proxy between Cloudflare and PHP and the connection address is not available, add only that proxy's CIDRs to `CLOUDFLARE_TRUSTED_PROXY_CIDRS`; never add visitor networks or a catch-all CIDR.
...
Pickupsheet is available worldwide except from North America as defined by the UN M49 geographic grouping: Northern America, Central America, and the Caribbean. The public corporate site remains globally available. The application applies this denylist to `/dhl/pickupsheet/*` and legacy `/pickupsheet/*` routes using the trusted Cloudflare `CF-IPCountry` header; missing, malformed, unresolved (`XX`), Tor (`T1`), or untrusted country data fails closed. Enable **Network > IP Geolocation** (or the visitor-location Managed Transform) so Cloudflare supplies that header. The server-managed switch remains:

```dotenv
PICKUPSHEET_GEO_RESTRICTION_ENABLED=true
```

Mirror the boundary in Cloudflare Zero Trust with a higher-priority **Block** policy whose **Include / Country** values contain the North American countries and territories, while retaining the existing identity-based Allow policy. Cloudflare evaluates multiple values inside one Include rule with OR logic. The application denylist remains the origin-side enforcement layer.
