...
This release adds migration `005_create_pickup_records_users.sql`. After deployment, temporarily set `RUN_MIGRATIONS=true`, make one controlled request to `/health`, open the admin user-management page to confirm the table is available, and immediately return `RUN_MIGRATIONS=false`. The runtime MySQL user should then have schema-changing privileges removed. Apply the same controlled process for future schema changes.
