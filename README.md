...
All Pickupsheet routes require the operator account configured with `PICKUPSHEET_LOGIN_USERNAME`, `PICKUPSHEET_LOGIN_NAME`, and a unique `PICKUPSHEET_LOGIN_PASSWORD` of at least 16 characters in the server-managed `.env`; credentials are never committed. The Check by value is not editable or accepted from the browser: it is assigned on the server from the authenticated operator's display name. `/pickupsheet/submissions` is a no-store view of the 100 most recent sheets. Authenticated operators can expand each reference, print or save an A4 landscape sheet as PDF, or download an Excel-compatible UTF-8 CSV. Print and export routes enforce the same protected session and query a single validated reference.
