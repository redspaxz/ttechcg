...
Pickupsheet is restricted to requests geolocated by Cloudflare to Cameroon (`CM`) or Nigeria (`NG`); the public corporate site remains globally available. In Cloudflare Zero Trust, create a reusable Access rule group with one **Include / Country** rule containing **Cameroon** and **Nigeria**, then add that group as a **Require / Rule group** condition on the Pickupsheet Allow policy. Retain the JumpCloud identity or group requirement in the same policy. Also enable **Network > IP Geolocation** (or the visitor-location Managed Transform) so Cloudflare sends `CF-IPCountry` to the origin. The application applies the same hard two-country boundary to `/dhl/pickupsheet/*` and legacy `/pickupsheet/*` routes and fails closed when the trusted header is absent, malformed, or contains any other country. These server-managed settings make the policy explicit:

```dotenv
PICKUPSHEET_GEO_RESTRICTION_ENABLED=true
```

Do not create separate Cameroon and Nigeria **Require / Country** conditions: separate Require conditions use AND logic and cannot both match one request. Use one reusable country rule group so the two countries are evaluated with OR logic.
