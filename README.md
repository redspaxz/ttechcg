...
Copy the client ID and the one-time client secret into the server-managed `.env`, then set the matching `JUMPCLOUD_OIDC_*` and `JUMPCLOUD_RBAC_*` values shown in `.env.example`. Use the issuer for the JumpCloud organisation's region: US `https://oauth.id.jumpcloud.com/`, EU `https://oauth.id.eu.jumpcloud.com/`, or India `https://oauth.id.in.jumpcloud.com/`. JumpCloud users must also be authorised to the SSO application; application access is denied by default. Local account login remains available alongside JumpCloud.

OIDC maps identities at sign-in and does not copy JumpCloud users into MySQL. Group or account changes therefore take effect on the next login or when the existing Pickupsheet session expires (one hour idle, eight hours absolute). Local accounts remain separately managed in Pickupsheet.
