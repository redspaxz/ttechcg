...
The pickup-sheet entry form remains public, and “Checked by” is a required field entered with each shipment. Stored records are fail-closed: `/dhl/pickupsheet/submissions`, individual print views, and CSV exports require HTTPS Basic credentials whose password hash exists only in the server-managed `.env`. The records view shows the 100 most recent sheets. Successful and denied records access is security-logged using a hashed client identifier and hashed record identifier. Legacy `/pickupsheet` URLs redirect permanently to the new `/dhl/pickupsheet` location.
