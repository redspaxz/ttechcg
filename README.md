...
This release uses migrations `005_create_pickup_records_users.sql` through `011_create_pickup_security_events.sql`. Authenticated workflows initialize missing account, audit, lifecycle, administrator-credential, user-name, session-activity, and security-event storage when the runtime MySQL user has the required schema permissions, supporting cPanel accounts without CLI access. Login and unauthenticated requests cannot create schemas. If the runtime MySQL user does not have `CREATE` and `ALTER` permission, use cPanel phpMyAdmin to execute migrations `005` through `011`, then keep `RUN_MIGRATIONS=false`.
