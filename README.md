...
Pickupsheet is restricted to requests geolocated by Cloudflare to Cameroon (`CM`) only; the public corporate site remains globally available. In Cloudflare Zero Trust, add **Cameroon** as a **Require / Country** condition on the Pickupsheet Allow policy and retain the JumpCloud identity or group requirement in the same policy. Also enable **Network > IP Geolocation** (or the visitor-location Managed Transform) so Cloudflare sends `CF-IPCountry` to the origin. The application applies the same hard Cameroon-only boundary to `/dhl/pickupsheet/*` and legacy `/pickupsheet/*` routes and fails closed when the trusted header is absent, malformed, or contains any other country. These server-managed settings make the policy explicit:
